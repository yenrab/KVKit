        ...
        */
        if(!qc.database){
            ...
            qc.database.transaction(function (tx) {  
               ...
               tx.executeSql('CREATE TABLE IF NOT EXISTS relationship (parent_uuid, child_uuid, child_table_name)');
               tx.executeSql('CREATE TABLE IF NOT EXISTS js_native_value (uuid, value)');
               tx.executeSql("INSERT OR REPLACE INTO js_native_value (id,value) VALUES (0,'false')");
               tx.executeSql("INSERT OR REPLACE INTO js_native_value (id,value) VALUES (1,'true')");
            });
        }
    }
}
...
qc.goORM = function(){
    if(qc.storeQueue.length > 0){
        ...
        if(aStorageRequest.is_single_request){//single request
            ...
            var arrayAttributeNativeValues = new Array();
            var arrayAttributeStorableValues = new Array();
            ...
            for(var attribute in aStorable){
                if(aStorable[attribute].constructor != Function && attribute != 'UUID'){
                    //console.log('attribute type: '+aStorable[attribute].constructor);
                    //console.log('object type: '+aStorable.constructor);
                    if(aStorable[attribute].constructor == Array){
                        var valueArray = aStorable[attribute];
                        var numValues = valueArray.length;
                        for(var i = 0; i < numValues; i++){
                            var aValue = valueArray[i];
                            var valueType = aValue.constructor;
                            if(valueType == Number || valueType == String || valueType == Date){
                                var anEntry = [qc.generateUUID(),aValue];
                                arrayAttributeNativeValues.push(anEntry);
                            }
                            else if(qc.isStorable(aValue)){
                                arrayAttributeStorableValues(aValue);
                            }
                        }
                    }
                    else{
               ...
               tx.executeSql(getCreationSQL,null,function(tx, resultSet) {
                                          ...
                                          if(resultSet.rows.length == 0){//table doesn't exist
                                                ...
                                                if(aStorable.eventType == 'delete'){
                                                    ...
                                                    tx.executeSql('SELECT child_uuid FROM relationship WHERE parent_uuid = ?',[UUID], null, function(tx, results){
                                                        //these are the values that are referred to by the element being deleted.
                                                        /*
                                                        * for each record in the result set see if there are other records where this child_uuid
                                                        * appears as a child_uuid.  If it does not then delete the child value from the js_native_value
                                                        * or the table in which it appears.
                                                        */
                                                        
                                                        
                                                        
                                                        tx.executeSql('DELETE FROM relationship WHERE parent_uuid = ? OR child_uuid == ?)',[UUID, UUID]);
                                                    });
                                                }
                                                else{
...
qc.isStorable = function(anObject){
    return anObject.UUID && anObject.syncType && anObject.eventType && anObject.updateTime && anObject.delete && anObject.delete.constructor == Function && anObject.store && anObject.store.constructor == Function;
}
